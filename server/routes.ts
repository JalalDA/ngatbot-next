...
export function registerRoutes(app: Express): Server {
  ...
  // Simple import services endpoint - NO MONITORING
  app.post("/api/smm/providers/:id/import-services", requireAuth, async (req, res) => {
    try {
      ...
      console.log(`Processing ${selectedServices.length} services sequentially...`);

      // Process services one by one to avoid overload
      for (const serviceData of selectedServices) {
        try {
          // Generate unique MID for this service
          const mid = generateMid(usedMids);
          usedMids.push(mid);

          // Create service with user-specific MID
          await storage.createSmmService({
            userId: user.id,
            providerId: provider.id,
            providerServiceId: serviceData.service.toString(),
            mid,
          ...
          importedCount++;
          console.log(`✅ Imported: ${serviceData.name}`);
        } catch (error) {
          console.error(`❌ Failed to import service ${serviceData.name}:`, error);
          errors.push(`${serviceData.name}: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
      }
